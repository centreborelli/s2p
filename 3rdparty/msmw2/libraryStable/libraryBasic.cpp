				 ...
				 std::vector<ParStruct2*>  &par)
{
	...
	{
		printf("\t%s",par[i]->name);
		if (par[i]->isoptional) printf(" (optional) ");
		printf("\t %s\n", par[i]->comment);
	}
				 ...
				 std::vector <ParStruct2*> & par)
{
   ...
   int optional_parameters=0;
	for(int i=0; i < npar; i++) { if (par[i]->isoptional) optional_parameters++; }
	
	if (argc - optind < npar - optional_parameters) {
		printf("\n%s: %s\n", pname, function);
		fprintf (stderr, "\nerror: incorrect number of parameters\n");
		printusage2(pname, gp,  opt,par);
		return 0;
