...
public class subpixLoG {
	 ...
	 */
	public static ImageFloat32 denseDisparity( ImageFloat32 rectLeft , ImageFloat32 rectRight ,
                                  ...
                                  int regionRadiusX, int regionRadiusY, double maxPerPixelError, int validateRtoL, double texture)
	{
      ...
      // regionSubpixelWta(DisparityAlgorithms whichAlg, int minDisparity, int maxDisparity, 
      ...
      // int validateRtoL, double texture, java.lang.Class<T> imageType) 
		StereoDisparity<ImageFloat32,ImageFloat32> disparityAlg =
				FactoryStereoDisparity.regionSubpixelWta(DisparityAlgorithms.RECT_FIVE,
						minDisparity, maxDisparity, regionRadiusX, regionRadiusY, maxPerPixelError, validateRtoL, texture, ImageFloat32.class);
      ...
      // recover disparity
      ImageFloat32 disp = disparityAlg.getDisparity();

      // fix problems with the output disparities
      final float[] data = disp.data;
...
//      System.out.print(args[2]);
      if (args.length < 3) {
         ...
         System.out.print("   subpix   im1.png im2.png out.pgm [mindisp(0) maxdisp(60) regionRadius(3) maxPerPixelError(20) validateRtoL(1) texture(0.2)]");
         return;
      }
...
//		rectify(distLeft,distRight,param,rectLeft,rectRight);
		ImageFloat32 distLeftF  = ConvertImage.convert(distLeft ,(ImageFloat32)null);
		ImageFloat32 distRightF = ConvertImage.convert(distRight,(ImageFloat32)null);


      // LoG
      distLeftF  = LoG(distLeftF);
      distRightF = LoG(distRightF);



		// compute disparity
      ImageFloat32 disparity = denseDisparity(distLeftF,distRightF,mindisp,maxdisp,regionRadius,regionRadius,maxPerPixelError,validateRtoL,texture);
      ...
      try {
         ...
         for(int y=0; y<disparity.height; y++) {
            for(int x=0; x<disparity.width; x++, n++) {
               out.write(String.format("%.8g ",disparity.get(x,y)));
               if(n==16) { out.write("\n"); n = 0; }
            }
         }
         out.close();
