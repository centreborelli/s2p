...
def pointing_correction(tile, i):
    ...
    if m is not None:  # m is the list of sift matches
        ...
        np.savetxt(
            os.path.join(out_dir, "center_keypts_sec.txt"),
            np.mean(m[:, 2:], 0),
            fmt="%9.3f",
        )
        if cfg["debug"]:
...
def rectification_pair(tile, i):
    ...
    img2 = cfg["images"][i]["img"]
    rpc2 = cfg["images"][i]["rpcm"]
    pointing = os.path.join(cfg["out_dir"], "global_pointing_pair_{}.txt".format(i))
    ...
    np.savetxt(
        os.path.join(out_dir, "disp_min_max.txt"), [disp_min, disp_max], fmt="%3.1f"
    )
...
def stereo_matching(tile, i):
    ...
    block_matching.compute_disparity_map(
        ...
        disp,
        mask,
        cfg["matching_algorithm"],
        disp_min,
        disp_max,
        timeout=cfg["mgm_timeout"],
...
def disparity_to_height(tile, i):
    ...
    height_map = triangulation.height_map(
        x,
        y,
        w,
        h,
...
def disparity_to_ply(tile):
    ...
    if cfg["images"][0]["clr"]:
        ...
        common.image_apply_homography(
            colors, cfg["images"][0]["clr"], np.loadtxt(H_ref), ww, hh
        )
    ...
    xyz_array, err = triangulation.disp_to_xyz(
        rpc1,
        rpc2,
        np.loadtxt(H_ref),
        np.loadtxt(H_sec),
...
def mean_heights(tile):
    ...
    for i in range(n):
        try:
            with rasterio.open(
                os.path.join(tile["dir"], "pair_{}".format(i + 1), "height_map.tif"),
                "r",
            ) as f:
                maps[:, :, i] = f.read(1)
    ...
    np.savetxt(
        os.path.join(tile["dir"], "local_mean_heights.txt"),
...
def global_mean_heights(tiles):
    """ """
    local_mean_heights = [
        np.loadtxt(os.path.join(t["dir"], "local_mean_heights.txt")) for t in tiles
    ]
    global_mean_heights = np.nanmean(local_mean_heights, axis=0)
    for i in range(len(cfg["images"]) - 1):
        np.savetxt(
            os.path.join(
                cfg["out_dir"], "global_mean_height_pair_{}.txt".format(i + 1)
            ),
...
def heights_fusion(tile):
    ...
    for i in range(len(cfg["images"]) - 1):
        x = np.loadtxt(
            os.path.join(cfg["out_dir"], "global_mean_height_pair_{}.txt".format(i + 1))
        )
...
def heights_to_ply(tile):
    ...
    out_crs = geographiclib.pyproj_crs(cfg["out_crs"])
    xyz_array = triangulation.height_map_to_xyz(
        height_map, cfg["images"][0]["rpcm"], x, y, out_crs
    )
...
def plys_to_dsm(tile):
    ...
    points, _ = ply.read_3d_point_cloud_from_ply(os.path.join(tile["dir"], "cloud.ply"))
    if len(points) == 0:
        return
    ...
    raster, profile = plyflatten_from_plyfiles_list(
        clouds, resolution=r, roi=roi, radius=cfg["dsm_radius"], sigma=cfg["dsm_sigma"]
    )
...
def main(user_cfg, start_from=0):
    ...
    if start_from > 0:
        assert os.path.exists(tiles_txt), (
            "start_from set to {} but tiles.txt is not found in '{}'. Make sure this is"
            " the output directory of a previous run.".format(
                start_from, cfg["out_dir"]
            )
        )
    else:
    ...
    if start_from <= 1:
        ...
        parallel.launch_calls(
            pointing_correction, tiles_pairs, nb_workers, timeout=timeout
        )
    ...
    if start_from <= 3:
        ...
        parallel.launch_calls(
            rectification_pair, tiles_pairs, nb_workers, timeout=timeout
        )
    ...
    if start_from <= 4:
        ...
        parallel.launch_calls(
            stereo_matching, tiles_pairs, nb_workers_stereo, timeout=timeout
        )
    ...
    if start_from <= 5:
        if n > 2:
            ...
            parallel.launch_calls(
                disparity_to_height, tiles_pairs, nb_workers, timeout=timeout
            )
            ...
            parallel.launch_calls(disparity_to_ply, tiles, nb_workers, timeout=timeout)
...
def read_config_file(config_file):
    ...
    if not os.path.isabs(user_cfg["out_dir"]):
        user_cfg["out_dir"] = make_path_relative_to_file(
            user_cfg["out_dir"], config_file
        )
    ...
    if (
        k in user_cfg
        and isinstance(user_cfg[k], str)
        and not os.path.isabs(user_cfg[k])
    ):
    ...
    if "exogenous_dem" in user_cfg and user_cfg["exogenous_dem"] is not None:
        if not os.path.isabs(user_cfg["exogenous_dem"]):
            user_cfg["exogenous_dem"] = make_path_relative_to_file(
                user_cfg["exogenous_dem"], config_file
            )
