...
def create_rejection_mask(disp, im1, im2, mask):
    ...
    common.run(
        [
            ...
            im1,
            tmp2,
            "x isfinite y isfinite z isfinite and and vmul",
            "-o",
            mask,
        ]
    )
...
def compute_disparity_map(
    ...
    if algo == "tvl1":
        tvl1 = "callTVL1.sh"
        common.run("{0} {1} {2} {3} {4}".format(tvl1, im1, im2, disp, mask), env)

    if algo == "msmw":
        bm_binary = "iip_stereo_correlation_multi_win2"
        common.run(
            "{0} -i 1 -n 4 -p 4 -W 5 -x 9 -y 9 -r 1 -d 1 -t -1 -s 0 -b 0 -o 0.25 -f 0 -P 32 -m {1} -M {2} {3} {4} {5} {6}".format(
    ...
    if algo == "msmw2":
        ...
        common.run(
            "{0} -i 1 -n 4 -p 4 -W 5 -x 9 -y 9 -r 1 -d 1 -t -1 -s 0 -b 0 -o -0.25 -f 0 -P 32 -D 0 -O 25 -c 0 -m {1} -M {2} {3} {4} {5} {6}".format(
                bm_binary, disp_min, disp_max, im1, im2, disp, mask
            ),
            env,
        )
    ...
    if algo == "msmw3":
        bm_binary = "msmw"
        common.run(
            "{0} -m {1} -M {2} -il {3} -ir {4} -dl {5} -kl {6}".format(
                bm_binary, disp_min, disp_max, im1, im2, disp, mask
            )
        )
    ...
    if algo == "mgm_multi_lsd":
        ...
        common.run(
            "qauto %s | \
                   ...
                   cut -d' ' -f1,2,3,4   | \
                   pview segments %d %d | \
                   plambda -  \"255 x - 255 / 2 pow 0.1 fmax\" -o %s"
            % (ref, width, height, wref),
            shell=True,
        )
        # image dependent weights based on lsd segments
    ...
    if algo == "micmac":
        ...
        micmac_disp = os.path.join(
            work_dir, "MEC-EPI", "Px1_Num6_DeZoom1_LeChantier.tif"
        )
        ...
        micmac_cost = os.path.join(work_dir, "MEC-EPI", "Correl_LeChantier_Num_5.tif")
