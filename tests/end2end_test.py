...
def compare_dsm(computed, expected, absmean_tol, percentile_tol):
    ...
    np.testing.assert_allclose(
        n_computed, n_expected, rtol=0.01, atol=100, verbose=True
    )
    ...
    print(
        "99th percentile abs difference",
        percentile,
...
def test_end2end_pair():
    end2end(
        data_path("input_pair/config.json"),
        data_path("expected_output/pair/dsm.tif"),
        0.025,
        1,
    )
...
def test_end2end_triplet():
    end2end(
        data_path("input_triplet/config.json"),
        data_path("expected_output/triplet/dsm.tif"),
        0.05,
        2,
    )
...
def test_mosaic(tmp_path):
    ...
    s2p_mosaic.main(
        os.path.join(test_cfg["out_dir"], "tiles.txt"),
        os.path.join(tmp_path, "height_map.tif"),
        "pair_1/height_map.tif",
    )

    computed = common.rio_read_as_array_with_nans(
        os.path.join(tmp_path, "height_map.tif")
    )
    expected = common.rio_read_as_array_with_nans(
        data_path("expected_output/triplet/height_map.tif")
    )
...
def test_distributed_plyflatten():
    ...
    # we assume that test_end2end_triplet has already run
    computed = common.rio_read_as_array_with_nans(
        os.path.join(test_cfg["out_dir"], "dsm.tif")
    )

    clouds_list = glob.glob(
        os.path.join(test_cfg["out_dir"], "tiles", "*", "*", "cloud.ply")
    )
    raster, _ = plyflatten_from_plyfiles_list(
        clouds_list, resolution=test_cfg["dsm_resolution"], roi=None
    )
    expected = raster[:, :, 0]
