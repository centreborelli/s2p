...
class TestRpcModel(TestWithDefaultConfig):
    ...
    def test_matches_from_rpc(self):
        ...
        rpc1 = rpc_model.RPCModel(data_path('input_pair/rpc_01.xml'))
        rpc2 = rpc_model.RPCModel(data_path('input_pair/rpc_02.xml'))

        test_matches = rpc_utils.matches_from_rpc(rpc1,rpc2,100,100,200,200,5)
        expected_matches = np.loadtxt(data_path('expected_output/units/unit_matches_from_rpc.txt'))
