...
def test_image_keypoints():
    ...
    descriptors.
    """
    computed = sift.image_keypoints(
        data_path("input_triplet/img_02.tif"), 100, 100, 200, 200
    )
...
def test_matching():
    ...
    """
    computed = sift.keypoints_match(
        np.loadtxt(data_path("units/sift1.txt")),
        np.loadtxt(data_path("units/sift2.txt")),
    )
    expected = np.loadtxt(data_path("expected_output/units/unit_keypoints_match.txt"))
    np.testing.assert_allclose(computed, expected, rtol=0.01, atol=0.1, verbose=True)
