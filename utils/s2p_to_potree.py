...
def test_for_potree(basedir):
    PotreeConverter = os.path.join(
        basedir, "PotreeConverter/build/PotreeConverter/PotreeConverter"
    )
    print("looking for:\n    %s" % PotreeConverter)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "s2pout", nargs="+", help=("path(s) to the s2p output directory(ies)")
    )
    parser.add_argument(
        "--outdir",
        metavar="potree_outdir",
        default=".",
    ...
    try:
        ...
        basedir = os.path.dirname(os.path.abspath(__file__))
        print(
            "You must download and compile PotreeConverter. Run the following commands:"
        )
        print("    > cd %s" % basedir)
        print(
